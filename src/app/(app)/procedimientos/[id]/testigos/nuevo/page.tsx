...
export default function NuevoTestigo() {
  ...
  async function manejarEnvio(evento: FormEvent) {
    ...
    setCargando(true);
    try {
      const nuevo = await api.post<{ id: string }>(`/procedimientos/${id}/testigos`, {
        primerNombre,
        primerApellido,
      });
      router.push(`/procedimientos/${id}/testigos/${nuevo.id}`);
    } catch (err) {
      setError(err instanceof ApiError ? err.message : "No fue posible crear el testigo.");
    } finally {
      setCargando(false);
    }
  }
  ...
  return (
    <div className="mx-auto max-w-lg">
      ...
      <h1 className="mt-3 font-display text-2xl text-institucional-950">Nuevo testigo</h1>
